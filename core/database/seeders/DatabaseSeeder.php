...
class DatabaseSeeder extends Seeder
{
    ...
    {
        $theme_json_file = base_path('assets/landlord/admin/demo-theme-asset/json-data/all-themes.json');

        if (!file_exists($theme_json_file)) {
            Log::warning('Theme JSON file not found: ' . $theme_json_file);
            return;
        }

        $theme_json_path = json_decode(file_get_contents($theme_json_file));
    ...
    {
        $gateway_json_file = base_path('assets/common/common-json-data/payment-gateways.json');

        if (!file_exists($gateway_json_file)) {
            Log::warning('Payment gateway JSON file not found: ' . $gateway_json_file);
            return;
        }

        $gateway_json_path = json_decode(file_get_contents($gateway_json_file));
    ...
    private function landlord_new_permission_seed()
    {
        $permissions = [
            ...
            'appearance-menu-manage',
            'appearance-country-list',
            'appearance-404-settings',
            'appearance-topbar-settings',
            'appearance-email-template',
            'appearance-login-register-settings',
            'appearance-maintenance-settings',
            'user-website-instruction-list',
            'user-website-instruction-create',
            'user-website-instruction-edit',
            'user-website-instruction-delete',
            'other-demo-data',
            'page-builder-demo-data',
            'plugin-manage'
        ];
