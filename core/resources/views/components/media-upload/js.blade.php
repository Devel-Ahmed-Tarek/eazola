...
if($route_name === 'landlord'){
    ...
    $file_delete_route = route($route_name.'.user.upload.media.file.delete');
    // file alt change
    $file_alt_change_route = route($route_name.'.user.upload.media.file.alt.change');
    // all file get
    $all_file_fetch_route = route($route_name.'.user.upload.media.file.all');
    // load more
    $file_load_more_route = route($route_name.'.user.upload.media.file.loadmore');
}
    ...
    ;(function () {
        ...
        $(document).ready(function (){
            ...
            $(document).on('click','#loadmorewrap',function (){
                ...
                $('#loadmorewrap button').text('{{__('Loading...88898989')}}');
                $.ajax({
                    type: "POST",
