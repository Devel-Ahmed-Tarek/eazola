...
@if(get_static_option('landlord_frontend_topbar_show_hide'))
    ...
    @php
        ...
        $condition_for_portfolio_inner = $theme == 'hotel-booking' && !request()->is('/') ? 'hotel_booking_inner_topbar' : '';
    @endphp

    <div class="header-top hotel_booking_topbar {{$theme}} {{$condition_for_portfolio_inner}}">
        <div class="container">
            <div class="row">
                <div class="col-xl-12">
                    <div class="d-flex justify-content-between flex-wrap align-items-center">
                        <div class="header-info-left">
                            @if(!empty(get_static_option('landlord_frontend_contact_info_show_hide')))
                                <ul class="listing">
                                    <li class="listItem header_contact"><i class="fa-solid fa-phone icon"></i>{{get_static_option('topbar_phone')}}</li>
                                    <li class="listItem header_contact"><i class="fa-solid fa-envelope icon"></i> {{get_static_option('topbar_email')}}</li>
                                </ul>
                            @endif
                        </div>
                                ...
                                <div class="select-language">
                                    <select class="niceSelectt tenant_languages_selector">
                                        @foreach(\App\Facades\GlobalLanguage::all_languages(\App\Enums\StatusEnums::PUBLISH) as $lang)
                                            ...
                                            @endphp
                                            <option class="lang_item" @selected(session()->get('lang') == $lang->slug) value="{{$lang->slug}}" >{{current($exploded)}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endif
