...
(function ($) {
    ...
    function getKbId(){ var v=parseInt(modalEl.getAttribute('data-kb-id')||'0',10); return v>0?v:0; }
    function getCurrentLang(){ return $('select[name="lang"]').val() || $('input[name="lang"]').val() || modalEl.getAttribute('data-ai-lang') || 'en'; }
    function showError(msg){ $('#kb-ai-error').removeClass('d-none').text(msg || @json(__('Request failed.'))); }
    function hideError(){ $('#kb-ai-error').addClass('d-none').text(''); }
    function getCsrf(){ return $('meta[name="csrf-token"]').attr('content'); }

    function getEditorHtml(){ var $el=$('textarea.summernote').first(); if(!$el.length) return ''; try{return $el.summernote('code')||'';}catch(e){return $el.val()||'';} }
    function setEditorHtml(html){ var $el=$('textarea.summernote').first(); if(!$el.length) return; try{$el.summernote('code',html);}catch(e){$el.val(html);} }
    ...
    $(document).on('click','.kb-ai-open-modal',function(){
        ...
        hideError();
        $('#kb_ai_topic,#kb_ai_instruction').val('');
        var id=getKbId();
        var $all=$('#kb_ai_all_langs');
        if(mode==='refine' && id===0){ $all.prop('checked',false).prop('disabled',true); }
        else { $all.prop('disabled',false); if(mode==='generate') $all.prop('checked',true); }
        if(mode==='generate'){ $('#kb-ai-panel-generate').removeClass('d-none'); $('#kb-ai-panel-refine').addClass('d-none'); }
        else { $('#kb-ai-panel-generate').addClass('d-none'); $('#kb-ai-panel-refine').removeClass('d-none'); }
        if(bsModal) bsModal.show();
    });

    $('#kb_ai_run_btn').on('click', function(){
        hideError();
        var allLangs=$('#kb_ai_all_langs').is(':checked');
        var id=getKbId();
        if(mode==='refine' && allLangs && id===0){ showError(@json(__('To improve all languages, save the knowledgebase first, then use this option on edit screen.'))); return; }
        ...
        else { payload.instruction=$('#kb_ai_instruction').val()||''; payload.current_content=getEditorHtml(); if(allLangs&&id>0) payload.knowledgebase_id=id; }
        ...
        $.ajax({url:assistUrl,method:'POST',data:JSON.stringify(payload),contentType:'application/json; charset=UTF-8',headers:{'X-CSRF-TOKEN':getCsrf(),'Accept':'application/json'}})
         .done(function(res){
            ...
            if(res.all_languages && res.translations){
                var $bulk=$('#kb_ai_bulk_translations_json');
