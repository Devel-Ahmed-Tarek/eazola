...
<nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
    <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
        <a class="navbar-brand brand-logo" href="{{ route('tenant.admin.dashboard') }}">
            @php
                $logo_type =
                    get_static_option('tenant_default_theme') == 'article-listing' ? 'site_white_logo' : 'site_logo';
            @endphp
    ...
    <div class="navbar-menu-wrapper d-flex align-items-stretch">
        ...
        <div class="tenant_info d-flex">
            @php
                ...
                $page_limit =
                    $permission_limit?->page_permission_feature == -1
                        ? __('Unlimited')
                        : $permission_limit?->page_permission_feature;
                $blog_limit =
                    $permission_limit?->blog_permission_feature == -1
                        ? __('Unlimited')
                        : $permission_limit?->blog_permission_feature;
                $product_limit =
                    $permission_limit?->product_permission_feature == -1
                        ? __('Unlimited')
                        : $permission_limit?->product_permission_feature;
                $storage_limit =
                    $permission_limit?->storage_permission_feature == -1
                        ? __('Unlimited')
                        : $permission_limit?->storage_permission_feature;

                $storage_count = get_tenant_storage_info('mb');
                $storage_remaining_percent = !empty($permission_limit?->storage_permission_feature)
                    ? 100 - ($storage_count / $permission_limit?->storage_permission_feature ?? 1) * 100
                    : 0;

                // Inventory Warnings
                $threshold_amount = get_static_option('stock_threshold_amount');

                $inventory_product_items = \Modules\Product\Entities\ProductInventoryDetail::where(
                    'stock_count',
                    '<',
                ...
                $inventory_product_items_id = !empty($inventory_product_items)
                    ? $inventory_product_items->pluck('product_id')->toArray()
                    : [];
                ...
                $all_products = \Modules\Product\Entities\Product::whereIn('id', $every_filtered_product_id)
                ...
                <div class="tenant_info_list">
                    ...
                    <div class="tenant_info_list_item">
                        ...
                        <span class="tenant_info_list_para">
                            <span
                                class="{{ $storage_remaining_percent >= $oneThirdOfStorage ? 'text-danger' : '' }}">{{ round($storage_count, 3) }}</span>
        ...
        <ul class="navbar-nav navbar-nav-right">
                        ...
                        <a class="dropdown-item preview-item"
                            ...
                            <div
                                ...
                                <h6 class="preview-subject mb-1 font-weight-normal">
                                    {{ __('You have message from') . ' ' }}
                                    <strong>{{ optional($message->form)->title }}</strong>
                                </h6>
                                <p class="text-gray mb-0"> {{ $message->created_at->diffForHumans() . ' ' }}
                                    @if ($message->status === 1)
            ...
            <li class="nav-item dropdown">
                <a class="nav-link count-indicator dropdown-toggle" id="notificationDropdown" href="#"
                    ...
                    <span
                        class="count-symbol {{ count($all_products) > 0 ? 'bg-danger' : '' }}">{{ count($all_products) > 0 ? count($all_products) : '' }}</span>
                </a>
                <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list"
                    ...
                    @forelse($all_products->take(10) as $product)
                        @php
                            ...
                            $variant = $product->inventoryDetail
                                ->where('stock_count', '<=', $threshold_amount)
                                ->first();
                        ...
                        <a class="dropdown-item preview-item"
                            ...
                            <div
                                ...
                                <p class="text-gray ellipsis mb-0 text-danger">
                                    {{ sprintf(__('Remaining stock is %u'), $stock) }} </p>
                            </div>
                        </a>
                    @empty
