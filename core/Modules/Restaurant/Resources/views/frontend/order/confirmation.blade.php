    ...
    <section class="confirmation-area section-bg-2 pat-100 pab-100">
        <div class="container">
            <div class="confirmation-contents center-text">
                ...
                <h4 class="confirmation-contents-title">  {{__('Your Food Order is confirmed')}}</h4>
                <p class="confirmation-contents-para"> {{__('Dear')}} {{@$data->name}}, {{__('Your reservation')}} <a href="javascript:void(0)"> {{@$data->reservation_id}} </a>{{__('has been confirmed. Please check your mail for reservation invoice. Thanks for reserving our hotel!')}} </p>
                <div class="btn-wrapper flex-btn mt-4 mt-lg-5">
                    <a href="{{url("/")}}" class="cmn-btn btn-outline-1 color-one"> {{__('Back to Home')}}</a>
                </div>
            </div>
        </div>
    </section>
