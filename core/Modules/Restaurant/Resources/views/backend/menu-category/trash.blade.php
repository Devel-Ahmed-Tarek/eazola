            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <h4 class="header-title mb-4">{{__('All Trashed Menu Categories')}}</h4>
                        @can('restaurant-menu-category-all')
                            <a href="{{route('tenant.admin.menu.category.all')}}" class="btn btn-sm btn-info btn-xs mb-3 mr-1 text-light">{{__('Back')}}</a>
                        @endcan
                        </div>
                        @can('restaurant-menu-category-trash_bulk_delete')
                            ...
                            <table class="table table-default">
                                ...
                                @foreach($all_category as $key => $category)
                                    <tr>
                                        ...
                                        <td>
                                            @can('restaurant-menu-category-trash_restore')
                                                <a class="btn btn-success" href="{{route('tenant.admin.menu.category.trash.restore', $category->id)}}">
                                                    {{__('Restore')}}
                                                </a>
                                            @endcan

                                            @can('restaurant-menu-category-trash_delete')
                                                <a class="btn btn-danger" href="{{route('tenant.admin.menu.category.trash.category.delete', $category->id)}}">
                                                    {{__('Delete')}}
                                                </a>
                                            @endcan
                                        </td>
...
@section('scripts')
    ...
    <x-media-upload.js/>
    <x-table.btn.swal.js/>
    @can('menu-category-delete')
        <x-bulk-action.js :route="route('tenant.admin.menu.category.trash.delete.bulk')"/>
    @endcan
@endsection
