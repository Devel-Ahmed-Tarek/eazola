...
@section('content')
    @php
        ...
        if(!isset($food_menu_attributes)){
         $food_menu_attributes = [];
        }
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <x-admin.header-wrapper>
                    <x-slot name="left">
                        <h3 class="heading-three fw-500"> {{ __("Update Menu") }} </h3>
                    </x-slot>
    ...
    <div class="dashboard-products-add bg-white radius-20 mt-4">
                ...
                <div class="row gy-4 d-flex align-items-start">
                    ...
                    <div class="col-xxl-10 col-xl-9 col-lg-12">
                        <div class="info-right-wrapper">
                            <form data-request-route="{{ route("tenant.admin.menu.manage.edit", $data['food_menu']->id) }}" method="post"
                                  id="product-create-form">
                                @csrf
                                <input type="hidden" name="lang" value="{{$data['default_lang']}}">
                                @csrf
                                <div class="form-button text-end">
                                    <button class="btn-sm btn btn-success">{{ __("Update Menu") }}</button>
                                </div>
                                <div class="info-right-inner">
                                    <div class="tab-content margin-top-10" id="v-pills-tabContent">
                                        <div class="tab-pane fade show active" id="v-general-info-tab" role="tabpanel"
                                            ...
                                            <x-restaurant::backend.menu.general-info :food-menu="$food_menu" :defaultLang="$data['default_lang']"/>
                                        </div>
                                        <div class="tab-pane fade" id="v-price-tab" role="tabpanel"
                                             aria-labelledby="v-price-tab">
                                            <x-restaurant::backend.menu.product-price :food-menu="$food_menu" />
                                        </div>
                                        <div class="tab-pane fade" id="v-tags-and-label" role="tabpanel"
                                             aria-labelledby="v-tags-and-label">
                                            <x-restaurant::backend.menu.tags-and-badge :food-menu="$food_menu" :defaultLang="$data['default_lang']"/>
                                        </div>
                                        <div class="tab-pane fade" id="v-images-tab" role="tabpanel"
                                             aria-labelledby="v-images-tab">
                                            <x-restaurant::backend.menu.product-image :food-menu="$food_menu" />
                                        </div>
                                        ...
                                        <div class="tab-pane fade" id="v-attributes-tab" role="tabpanel"
                                            ...
                                            <x-restaurant::backend.menu.product-attribute :is-first="true"
                                                                                          ...
                                                                                          :food-menu-attributes="$data['food_menu']->food_menu_attributes"
                                                                                          :all-attribute-values="$uniqueAllTerms"
                                        ...
                                        <div class="tab-pane fade" id="v-categories-tab" role="tabpanel"
                                            ...
                                            <x-restaurant::backend.menu.categories :sub-categories="$data['sub_categories']" :categories="$data['categories']" :food-menu="$food_menu" :defaultLang="$data['default_lang']"/>
                                        </div>

                                        <div class="tab-pane fade" id="v-meta-tag-tab" role="tabpanel"
                                             aria-labelledby="v-meta-tag-tab">
                                            <x-restaurant::backend.menu.meta-seo :default-lang="$data['default_lang']" :meta-data="$data['food_menu']->metaData"/>
                                        </div>
                                        <div class="tab-pane fade" id="v-settings-tab" role="tabpanel"
                                             aria-labelledby="v-settings-tab">
                                            <x-restaurant::backend.menu.settings :menu-taxes="$data['menu_taxes']" :food-menu="$food_menu" />
                                        </div>
                                        <div class="tab-pane fade" id="v-policy-tab" role="tabpanel"
                                             aria-labelledby="v-policy-tab">
                                            <x-restaurant::backend.menu.policy :food-menu="$food_menu" :default-lang="$data['default_lang']" />
                                        </div>
                                    </div>
                                </div>
                            </form>
        ...
        @section('scripts')
            ...
            <x-media-upload.js/>
            <x-summernote.js/>
            <x-restaurant::backend.menu.variant-info.js :colors="$data['product_colors']" :sizes="$data['product_sizes']"
                                                        :all-attributes="$data['all_attribute']"/>
            ...
            <script>
                ...
                (function($) {
                    ...
                    $(document).ready(function () {
                        ...
                        $(document).ready(function ()
                        {
                            $('.item_attribute_name').each(function () {

                                let terms = $(this).find('option:selected').data('terms');
                                let selectedId = $(this).find('option:selected').data('id');
                                let terms_html = '';
                                ...
                                terms.forEach(function (term)
                                {
                                    ...
                                    data.append("menu_attribute_id", selectedId);
                                    ...
                                    send_ajax_request("post", data, '{{ route('tenant.admin.menu.category.sub-category') }}', function () {
                                        ...
                                        if (term === data)
                                        {
                                            terms_html += '<option value="' + term + '" selected>' + term + '</option>';
